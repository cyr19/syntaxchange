...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        while index < len(gold_ud.characters) and index < len(system_ud.characters):
            ...
            #print(system_c)
            #raise ValueError
            #if gold_c.lower() == system_c.lower() or (not gold_c.isalpha() and not system_c.isalpha()):
            #if gold_c.lower() == system_c.lower() or not gold_c.isalpha():
            #print(f"{gold_c} - {system_c} - {re.match(r'[A-Za-z]', string=gold_c)}")
            '''
            if gold_c.lower() == system_c.lower() or (not re.match(r"[A-Za-z]", string=gold_c) or ):
                index += 1
            else:
                break
            '''
        ...
        #print('loop done!!!!')
        #if '<UNK>' not in "".join(map(_encode, system_ud.characters[index:index + 20])):
        #if '<UNK>' not in "".join(map(_encode, system_ud.characters[index:index + 20])):
...
def load_conllu_file(path, discarded=[]):
    ...
    if len(discarded) > 0:
        ...
        tmp = path
    #assert len(sents) =len(sents) - len(discarded)
